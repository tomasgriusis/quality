...
DamisService::DamisService(InitDamisServiceFile* passedFile)
{
    DamisService::serveFile = passedFile;
    DamisService::intClassAttributes.reserve(0);
    DamisService::rawDoubleData.reserve(0);
    DamisService::stringClassAttributes.reserve(0);
    DamisService::initialize();
...
std::vector<std::string> DamisService::prepareAttributeSection(std::vector<std::string> attributeNameList,  std::vector<std::string> attributeTypeList, std::vector<std::string> possibleClasses)    //concats attribute + class
{
    ...
    if (attributeNameList.size() > 0)
    {
        ...
        if (!possibleClasses.empty())
        {
            LOG(INFO) <<"Faund type and class information (string string string)";
...
std::vector<std::vector<std::string>> DamisService::prepareDataSection(std::vector<std::vector<double>> dataSection,  std::vector<std::string> classAttribute)  //concats data + class
{
...
void DamisService::writeDataToFile(std::string path, std::vector<std::vector<std::string>> data, std::vector<std::string> attributeData)
{
